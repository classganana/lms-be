...
export class SourceCodeResponseDto {
  ...
  @ApiProperty({ example: 'ACTIVE', enum: ['ACTIVE', 'INACTIVE'], description: 'Source code status' })
  status: 'ACTIVE' | 'INACTIVE';

  @ApiProperty({ example: '2026-01-22T16:07:52.623Z', description: 'Activation timestamp' })
  activatedAt: Date;

  @ApiProperty({ example: null, nullable: true, description: 'Deactivation timestamp' })
...
export class InfluencerResponseDto {
  ...
  @ApiProperty({ type: [SourceCodeResponseDto], description: 'List of source codes' })
