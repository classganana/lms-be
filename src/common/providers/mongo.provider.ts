  ...
  providers: [
    {
      ...
      useFactory: async (configService: ConfigService): Promise<Db> => {
        const uri = configService.get<string>('database.uri') || process.env.MONGODB_URI;
        const dbName = configService.get<string>('database.dbName') || process.env.MONGODB_DB || 'lead-management';
        ...
        if (allowInvalidTLS) {
          ...
          Logger.warn('MONGODB_TLS_ALLOW_INVALID is enabled — TLS certificate validation is disabled');
        }
        ...
        try {
          ...
          try {
            ...
            if (isSrv) {
              ...
              Logger.log(`SRV connection failed; attempting direct connect to SRV hosts for ${host}`);
              ...
              if ((dns as any) && (dns as any).promises && typeof (dns as any).promises.resolveSrv === 'function') {
                resolveSrvFn = (name: string) => (dns as any).promises.resolveSrv(name);
              } else {
                try {
                  // Try require('dns').promises
                  // eslint-disable-next-line @typescript-eslint/no-var-requires
                  const dnsReq = require('dns');
                  if (dnsReq && dnsReq.promises && typeof dnsReq.promises.resolveSrv === 'function') {
                    resolveSrvFn = (name: string) => dnsReq.promises.resolveSrv(name);
                  }
                } catch (e) {
                  try {
                    ...
                    if (dnsPromises && typeof dnsPromises.resolveSrv === 'function') {
                      resolveSrvFn = (name: string) => dnsPromises.resolveSrv(name);
                    }
                  } catch (ee) {
                    // leave undefined
                  }
              ...
              if (!resolveSrvFn) {
                throw new Error('DNS SRV resolution not available in this Node runtime');
              }
              ...
              for (const rec of records) {
                ...
                Logger.log(`Attempting direct connect to ${rec.name}:${rec.port}`);
                try {
                  const client = await MongoClient.connect(hostUri, { directConnection: true, tls: true, serverSelectionTimeoutMS: 5000 });
          ...
          if (allowInvalidTLS) {
            try {
              Logger.log('Attempting MongoDB connect with TLS validation disabled (debug fallback)');
              const client = await MongoClient.connect(uri, { ...connectOpts, tlsAllowInvalidCertificates: true, tlsAllowInvalidHostnames: true });
              Logger.log('MongoDB connected with TLS validation disabled');
              return client.db(dbName);
            } catch (secondErr) {
              logAndWrap('Error connecting DB with TLS invalid allowed (fallback)', secondErr);
            }
          }
